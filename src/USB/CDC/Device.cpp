...
void tud_cdc_line_state_cb([[maybe_unused]] uint8_t inst, [[maybe_unused]] bool dtr, [[maybe_unused]] bool rts)
{
	debug_d("%s(%u, DTR %u, RTS %u)", __FUNCTION__, inst, dtr, rts);
}

// Invoked when line coding is change via SET_LINE_CODING
void tud_cdc_line_coding_cb([[maybe_unused]] uint8_t inst, [[maybe_unused]] cdc_line_coding_t const* p_line_coding)
{
...
void tud_cdc_send_break_cb(uint8_t inst, uint16_t)
{
	auto dev = getDevice(inst);
	if(dev) {
